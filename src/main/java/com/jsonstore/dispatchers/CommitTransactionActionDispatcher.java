...
public class CommitTransactionActionDispatcher extends BaseActionDispatcher {
	...
	public CommitTransactionActionDispatcher(android.content.Context context) {
		super("commitTransaction", context);
		addParameter(CommitTransactionActionDispatcher.PARAM_OPTIONS, false, JSONStoreParameterType.OBJECT);
	}

	@Override
	public PluginResult actionDispatch(JSONStoreContext jsContext) {
		try{
			JSONStore.getInstance(getAndroidContext()).commitTransaction();
		}
