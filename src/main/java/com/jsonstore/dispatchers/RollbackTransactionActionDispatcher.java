...
public class RollbackTransactionActionDispatcher extends BaseActionDispatcher {
	private static final String PARAM_OPTIONS = "options"; //$NON-NLS-1$

	public RollbackTransactionActionDispatcher(android.content.Context context) {
		super("rollbackTransaction", context);
		addParameter(RollbackTransactionActionDispatcher.PARAM_OPTIONS, false, JSONStoreParameterType.OBJECT);
	}
	...
	public PluginResult actionDispatch(JSONStoreContext js_context) throws Throwable {
		...
		try{
			JSONStore.getInstance(getAndroidContext()).rollbackTransaction();
		}
		catch(JSONStoreNoTransactionInProgressException e){
			return new PluginResult (PluginResult.Status.ERROR, DatabaseConstants.RC_NO_TRANSACTION_IN_PROGRESS);
		}
		catch(JSONStoreTransactionFailureException e){
			return new PluginResult (PluginResult.Status.ERROR, DatabaseConstants.RC_TRANSACTION_FAILURE_DURING_ROLLBACK);
		}
