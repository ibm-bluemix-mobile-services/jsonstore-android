...
public class CollectionAdvancedFindNotRightLikeTest extends InstrumentationTestCase {
	...
	public void testAdvancedFindNotRightLikeStringProveFuzzy() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotRightLike("name", "carlos");
	...
	public void testAdvancedFindNotRightLikeIntegerProveFuzzy() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotRightLike("age", 2);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
		...
		assertTrue(results.get(3).getJSONObject("json").getString("name").equals("nana"));
	
		databaseDestroy();
	}
	
	public void testAdvancedFindNotRightLikeASF() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotRightLike("l337", "4");
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindNotRightLikeMultiple() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotRightLike("age", "4");
		part.addNotRightLike("name", "carlos");
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
