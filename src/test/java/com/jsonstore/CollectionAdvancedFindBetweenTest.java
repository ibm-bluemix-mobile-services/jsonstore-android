...
public class CollectionAdvancedFindBetweenTest extends InstrumentationTestCase {
	...
	public void testAdvancedFindBetweenIntegerNone() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addBetween("age", 30, 40);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindBetweenIntegerMultiple() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addBetween("age", 20, 25);
		part.addBetween("gpa", 2.5, 4.0);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindBetweenBackwards() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addBetween("age", 25, 1);
		query.addQueryPart(part);
	...
	public void testAdvancedFindBetweenNull() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		...
		try {
			JSONStoreQueryParts query = new JSONStoreQueryParts();
			JSONStoreQueryPart part = new JSONStoreQueryPart();
			part.addBetween("age", 1, null);
			query.addQueryPart(part);
			col.findDocuments(query);
