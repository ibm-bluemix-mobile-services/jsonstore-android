...
public class CollectionAdvancedFindEqualTest extends InstrumentationTestCase {
	...
	public void testAdvancedFindEqualsIntegerNone() throws Throwable {
		JSONStoreCollection col = databaseSetup();
	...
	public void testAdvancedFindEqualsIntegerMultiple() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addEqual("age", 25);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindEqualsStringMultiple() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addEqual("last_name", "ortman");
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
		
		assertEquals(2, results.size());
		assertTrue(results.get(0).getJSONObject("json").getString("name").equals("mike"));
		assertTrue(results.get(1).getJSONObject("json").getString("name").equals("andrew"));
		
		databaseDestroy();
	}
	
	public void testAdvancedFindEqualsBooleanSingle() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addEqual("active", true);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindEqualsStringProveNotFuzzy() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addEqual("last_name", "ort");
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindEqualsASF() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addEqual("l337", 42);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
