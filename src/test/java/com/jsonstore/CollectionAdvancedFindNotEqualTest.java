...
public class CollectionAdvancedFindNotEqualTest extends InstrumentationTestCase {
	...
	public void testAdvancedFindNotEqualsNumber() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotEqual("gpa", 4.8);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindNotEqualsStringMultiple() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotEqual("last_name", "ortman");
		part.addNotEqual("name", "andrew");
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindNotEqualsStringProveNotFuzzy() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotEqual("last_name", "ort");
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindNotEqualsASF() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotEqual("active", true);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
