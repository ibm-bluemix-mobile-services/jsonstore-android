...
public class CollectionAdvancedFindNotLeftLikeTest extends InstrumentationTestCase {
	...
	public void testAdvancedFindNotLeftLikeStringProveFuzzy() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotLeftLike("name", "Anderu");
		...
		assertTrue(results.get(3).getJSONObject("json").getString("name").equals("mikenana"));
		
		databaseDestroy();
	}
	
	public void testAdvancedFindNotLeftLikeIntegerProveFuzzyEmpty() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotLeftLike("age", 2);
		part.addNotLeftLike("age", 1);
		part.addNotLeftLike("age", 5);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
	...
	public void testAdvancedFindNotLeftLikeIntegerProveFuzzySelect() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		...
		assertTrue(results.get(0).getJSONObject("json").getString("name").equals("nana is a coder so is carlos"));
		assertTrue(results.get(1).getJSONObject("json").getString("name").equals("mikenana"));
		
		databaseDestroy();
	}
	
	public void testAdvancedFindNotLeftLikeIntegerASF() throws Throwable {
		JSONStoreCollection col = databaseSetup();
		
		JSONStoreQueryParts query = new JSONStoreQueryParts();
		JSONStoreQueryPart part = new JSONStoreQueryPart();
		part.addNotLeftLike("l337", 2);
		query.addQueryPart(part);
		List<JSONObject> results = col.findDocuments(query);
